...
def metadata_artists():
    if request.method == "POST":
        ...
        if action == "change_real_artist":
            ...
            if new_name == raw_data['name'] or not new_name:
            ...
            if status:  # only when new artist is created
                ...
                new_artist.save()
                logger.info(
                    f"new artist {new_artist.name} created with old artist {old_artist.name}'s artist_info_json"
                )
            ...
            for album in albums:
                ...
                album.save()
                logger.info(
                    f"change album {album.name}'s artist to new artist {new_artist.name}"
                )
            ...
            for track in tracks:
                ...
                track.save()
                logger.info(
                    f"change track {track.title}'s artist to new artist {new_artist.name}"
                )
            ...
            for rel in artist_albums_relations:
                ...
                rel.save()
                logger.info(
                    f"change artist_album relation {rel.id}'s artist to new artist {new_artist.name}"
                )
            old_artist.real_artist = new_artist
            old_artist.save()
            logger.info(
                f"change old artist's real_artist to new artist {new_artist.name}"
            )
